...
export class ProfileService {
  ...
  protected convertDateFromClient(profile: IProfile): IProfile {
    return Object.assign({}, profile, {
      created: profile.created?.isValid() ? profile.created.toJSON() : undefined,
      updated: profile.updated?.isValid() ? profile.updated.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((profile: IProfile) => {
        profile.created = profile.created ? dayjs(profile.created) : undefined;
        profile.updated = profile.updated ? dayjs(profile.updated) : undefined;
      });
    }
    return res;
  }
